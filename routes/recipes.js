...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    if (!fs.existsSync(uploadDir)) {
      ...
      console.log('Created uploads/images directory:', uploadDir); // Debug
    }
    cb(null, uploadDir);
  },
  filename: (req, file, cb) => {
...
router.get('/user', auth, async (req, res) => {
  try {
    const recipes = await Recipe.find({ createdBy: req.user.id })
      .select('title prepTime image') // Explicitly select image field
      .populate('createdBy', 'name');
    console.log('Fetched recipes for user', req.user.id, ':', JSON.stringify(recipes, null, 2)); // Debug
    res.json(recipes);
  } catch (err) {
...
router.post('/', auth, upload.single('image'), async (req, res) => {
  const { title, prepTime } = req.body;
  console.log('Received file:', req.file ? req.file.originalname : 'No file'); // Debug file upload
  const image = req.file ? `/uploads/images/${req.file.filename}` : null; // Use null instead of ''
  ...
  try {
    ...
    await recipe.save();
    console.log('Saved recipe with image:', image); // Debug saved path
