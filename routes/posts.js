...
router.post('/:id/comment', auth, async (req, res) => {
  ...
  try {
    ...
    // Get the commenter from the last comment
    const latestComment = populatedPost.comments[populatedPost.comments.length - 1];
    const commenterName = latestComment?.user?.name || 'Unknown User';

    if (post.user.toString() !== req.user.id) {
      const notification = new Notification({
        user: post.user,
        message: `${commenterName} commented on your post: "${text}"`,
      });
      await notification.save();

      const io = req.app.get('socketio');
      if (io) {
        console.log('Emitting newNotification to:', post.user.toString(), 'with message:', notification.message);
